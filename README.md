...
3. 이진 검색 : [소스](./basic/algorithm02/app02/app02.c)
    - 데이터가 키값으로 이미 정렬(sort) 되어 있다는 가정하에서 시작 - 단점
    - 찾는 요소가 있는 없든 검색횟수가 비약적으로 줄어듬
...
2. 기술
    - `기초프로그래밍`, `알고리즘`, 통신프로그램, 데이터분석, 웹개발, 윈앱개발, IoT
...
3. `큐` : [소스](./basic/algorithm03/app01/app01.c)
    - 양쪽이 다 열려있어서 한쪽에서 데이터를 추가, 반대에서 데이터를 꺼내는 자료구조
    - 실행활 예 영화관 티켓줄, 버스대기줄, 식당대기줄,...
...
1. 의미 - Recursive(재귀)
    ...
    - 함수나 알고리즘의 자신의 함수를 호출하여, 더 작은 하위 문제를 해결하는 프로그램 기법 
...
3. 재귀 분석 : [소스](./basic/algorithm03/app04/app04.c)
    ...
    ![alt text](image-3.png)
        ...
        - recur(6) : recur(5) `6` recu(4) -> `1 2 3 1 4 1 2 5 1 2 3 1` 6 `1 2 3 1 4 1 2`
...
5. `메모이제이션!` : [소스](./basic/algorithm04/app01/app01.c)
    - 특정 조건(재귀, 동일한 부분문제 반복, 입력크기 규모)에서 매우 강력한 성는 향상 기법
    - 이미 계산한 결과를 저장해두고 다시 호출하면 재계산하지 않고 저장된 결과를 반환, 사용
    - 동적계획법에 많이 사용
    - 단점 - 메모리를 매우 많이 사용. 프로그램이 비정상 종료 될 수 있음 동적할당으로 변경 필수
    - 시간 복잡도(처리시간)를 줄이고 공간복잡도(메모리사용량)를 늘리는 방식

    ![alt text](image-6.png)

### 정렬 알고리즘

1. 데이터를 기준값에 따라 오름차순(작은값부터 큰값으로), 내일차순(큰값부터 작은값으로 데이터 나열)
...
5.셸정렬
    ...
    - 삽입정렬이 완전 뒤죽박죽일때 시간복잡도가 좋자 않음
    - 시간복잡도 : 평균 $0(n^{1.3})$ ~ $0(n^{1.5})$, 최악 $0(n^2)$, 최선 $0(n log n)$

6.퀵정렬
    - 기준값을 정하고, 작은값은 왼쪽, 큰값은 오르쪽으로 나눠서 재귀적으로 정렬방식
...
1.부루트-포스 - 모든 문자가 일치하는지 전부 비교하는 방법
    - 비밀번호 알아내기에서는 횟수제한 등으로 막힘

2. KMP - 부루트-포스 방법에서 패턴 형태로 변경된 비교 방법

3. 보이어-무어 - 부루트-포스와 KMP 상에서 패턴으로 옮길 크기 정해서 검색방법

### 리스트, 트리 자료구조

1. 데이터를 순서대로 나열한 자료구조 - 배열(선형 리스트)과 동일
    - 배열 사용할 경우 중간에 데이터가 빠지거나 추가되면 전부 옮기는 추가 처리필요
...
1. 검색과 데이터 추가, 삭제가 효율적으로 사용할 수 있는 자료구조
    ...
    - 자료 접금(검색)이 $0(1)$ 정도로 매우 빠름
    - 해시 함수를 통해서 만들어진 key와 실제 값(value)를 같이 저장
   
2. 동일한 key가 만들어저 충돌 가능할 단점 존재